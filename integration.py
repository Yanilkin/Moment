...
def f(L,t):
	...
	c = 1.0/dsizes*L0 + L1*(sizes-sizes_av)/(2*sigma) # Concentation c(xi)
	c_m1 = 1.0/dsizes*L0 + L1*(sizes-dsizes+1-sizes_av)/(2*sigma) # Concentation c(x(i-1) + 1)
...
if __name__ == "__main__":
	...
	results = odeint(f,p.L_in/scale,t)
	L0 = results[:,0:max_n]*scale/dsizes # Zero moment function
	L1 = results[:,max_n:]*scale/(2*sigma) # First moment function
	...
	fo.write('\n')
	i = 0
	discretization = int(p.integ_steps/p.nframes)
	for rows in L0:
